...
function setupTable(difficulty) {
    ...
    var vertical_space_between_cards = 200;
    var horizontal_spacing_between_cards = 200;

    var card_class = 'element';
    if (deck_size > 40) {
        card_class = 'element smaller_card';
        vertical_space_between_cards = 160;
        horizontal_spacing_between_cards = 160;
    }
    ...
    for (var i = 0; i < deck_size; i += 1) {
        ...
        var rowCount = Math.ceil(deck_size / table_width);
...
function updateView() {
    if (gameState == GameState.Setup) {
        $('#game_setup').removeClass('hidden');
    }
    else {
        $('#game_setup').addClass('hidden');
    }

    if (gameState == GameState.PreGameCountdown) {
        $('#pregame_countdown').removeClass('hidden');
    }
    else {
        $('#pregame_countdown').addClass('hidden');
    }

    if (gameState == GameState.Game) {
        $('#ingame_ui').removeClass('hidden');
    }
    else {
        $('#ingame_ui').addClass('hidden');
    }

    if (gameState == GameState.PostGame) {
        $('#post_game_menu').removeClass('hidden');
    }
    else {
        $('#post_game_menu').addClass('hidden');
    }
}